...
import SupportModal from './SupportModal'; // <--- Importamos o Modal

export default function Header() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isSupportOpen, setIsSupportOpen] = useState(false); // <--- Estado do Modal de Atendimento

  // Bloqueia a rolagem quando qualquer menu abre
  useEffect(() => {
  ...
  return (
    <>
      <header className="bg-[#D2B572] text-white shadow-md relative z-50 font-sans">
        ...
        <div className="lg:hidden flex items-center justify-between px-4 py-4 relative z-30">
          ...
          <div className="flex-1 flex justify-center w-1/2">
            <Link to="/" className="block">
              <img src={logoNav} alt="Sapato de Madame" className="h-14 w-auto object-contain" />
            </Link>
          </div>
        ...
        <div className="hidden lg:flex max-w-[1600px] mx-auto px-12 items-center justify-between gap-8 py-4">
          ...
          <div className="flex items-center gap-8 flex-shrink-0">
            {/* BOTÃO ATENDIMENTO DESKTOP - AGORA ABRE O MODAL */}
            <button 
              onClick={() => setIsSupportOpen(true)} // <--- Ação aqui
              className="flex flex-col items-center gap-1 hover:text-[#997617] transition-colors group"
            >
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto py-4 px-5 space-y-6">
              <div className="relative">
                  <input type="text" placeholder="Buscar..." className="w-full bg-slate-100 text-slate-800 text-sm py-3 px-4 pl-10 rounded-sm focus:outline-none focus:ring-1 focus:ring-[#D2B572]"/>
                  <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
              </div>
              <nav className="flex flex-col">
                  {['Home', 'Blusas', 'Camisas', 'Shorts', 'Saias', 'Calças', 'Casacos', 'Conjuntos', 'Outlet', 'Vestidos', 'Acessórios'].map((item) => (
                      <Link key={item} to={`/category/${item.toLowerCase()}`} onClick={() => setIsMenuOpen(false)} className="flex items-center justify-between py-3.5 text-slate-700 hover:text-[#D2B572] border-b border-slate-100 text-sm uppercase tracking-wide font-medium">
                          {item}
              ...
              <div className="space-y-4 pt-2 pb-8">
                  ...
                  {/* BOTÃO ATENDIMENTO MOBILE - AGORA ABRE O MODAL */}
                  <button 
                    onClick={() => { setIsMenuOpen(false); setIsSupportOpen(true); }} // Fecha menu e abre modal
                    className="flex items-center gap-3 text-slate-600 hover:text-[#D2B572] w-full"
                  >
      ...
      {/* O MODAL FICA AQUI, FORA DO HEADER MAS DENTRO DO COMPONENTE */}
